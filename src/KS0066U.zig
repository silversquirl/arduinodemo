...
pub fn init(comptime enable: anytype, comptime rs: anytype, comptime d4: anytype, comptime d5: anytype, comptime d6: anytype, comptime d7: anytype) KS0066U(ComptimePin(enable), ComptimePin(rs), ComptimePin(d4), ComptimePin(d5), ComptimePin(d6), ComptimePin(d7)) {
    return KS0066U(ComptimePin(enable), ComptimePin(rs), ComptimePin(d4), ComptimePin(d5), ComptimePin(d6), ComptimePin(d7))
        .init(.{}, .{}, .{}, .{}, .{}, .{});
}
pub fn KS0066U(comptime ENABLE: anytype, comptime RS: anytype, comptime D4: anytype, comptime D5: anytype, comptime D6: anytype, comptime D7: anytype) type {
    // const PINS = .{ D4, D5, D6, D7 };
    const Data = struct { d4: D4, d5: D5, d6: D6, d7: D7 };
    const set_bits = brk: while (true) {
        if (@hasDecl(D4, "singleton") and @hasDecl(D5, "singleton") and @hasDecl(D6, "singleton") and @hasDecl(D7, "singleton")) {
            const d4 = D4.singleton();
            const d5 = D5.singleton();
            const d6 = D6.singleton();
            const d7 = D7.singleton();
            if (d4.data_register() == d5.data_register() and d4.data_register() == d6.data_register() and d4.data_register() == d7.data_register()) {
    ...
    return struct {
        ...
        pub fn writeBytesNTimes(self: @This(), byte: []const u8, n: usize) Error!void {
            for (0..n) |_| {
