    ...
    return struct {
        ...
        pub fn use() @This() {
            comptime {
                ...
                if (!(std.mem.eql(u8, variant.to_std().name, @import("builtin").target.cpu.model.name) or (cpu_name[cpu_name.len - 1] == 'p' and std.mem.eql(u8, variant.to_std().name, cpu_name[0 .. cpu_name.len - 1])))) {
    ...
    pub fn pins(self: Package) []const Pin {
        switch (self) {
            .PDIP28 => return &.{
                ...
                .VCC, .GND,  .PB6,  .PB7, .PD5, .PD6,
                .PD7, .PB0,  .PB1,  .PB2, .PB3, .PB4,
                .PB5, .AVCC, .AREF, .GND, .PC0, .PC1,
                .PC2, .PC3,  .PC4,  .PC5,
            },
...
const mem_blocks: []const Block = blocks: while (true)
{
    ...
    break :blocks blocks ++ @as([]const Block, &.{ .{ 0x60, &.{
...
fn make_mem_abi(comptime blocks: []const Block) type {
    ...
    for (blocks) |block| {
        ...
        if (addr > current_offset) {
            ...
            // @compileLog(name);
            fields = fields ++ .{.{ .name = name, .type = @Type(std.builtin.Type{ .Array = .{ .child = u8, .len = addr - current_offset, .sentinel = null } }), .default_value = null, .is_comptime = false, .alignment = 0 }};
        }
        const data = block[1];
        ...
        for (0..registers) |n| {
            const name = data[n * 9];
            fields = fields ++ .{
                .{ .name = name, .type = register_type(data[n * 9 + 1 .. @min((n + 1) * 9, block[1].len)]), .default_value = null, .is_comptime = false, .alignment = 0 },
            };
        }
    }
    const typeInfo: std.builtin.Type = .{ .Struct = .{
        .layout = .Extern,
        .fields = fields,
        .decls = &.{},
        .is_tuple = false,
...
fn Registers(comptime register_blocks: []const Block) type {
    ...
    return @Type(.{ .Struct = .{
        .fields = registers[0..i],
        .layout = .Auto,
        .decls = &.{},
        .is_tuple = false,
    } });
}
fn make_registers(comptime register_blocks: anytype) Registers(register_blocks) {
    return .{};
}
